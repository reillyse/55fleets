class ImageBuilder < ActiveJob::Base
    ...
    begin
      ...
      Timeout.timeout(ENV['BUILD_TIMEOUT'] || 1200) do
        ...
        Machine.transaction do
          pod.with_lock do
            @machine =
              App.build_system.pods.first.machines.running.free.lock.first
            if @machine.nil?
              # if we don't have a builder we wait, unless we don't have any build machines
              # and haven't started a spot fleet in 20 minutes
              Rails
                .logger.warn 'No Builder machines available. Re-enqueue in 1 minute'
            ...
            @machine.build_notes =
              "Building pod #{pod.id} for app #{pod.app.name} at #Time"
            @machine.save!
          end
        ...
        t.connect [
                    ...
                    "ssh-agent bash -c 'ssh-add deploy_key ; rm -rf deploy; git clone -b master #{
                      repo.url
                    } deploy'",
                    'rm -rf deploy_key'
                  ]
        ...
        t1 =
          Thread.new do
              ...
              pod.before_hooks.each_line do |hook|
                ...
                if pod.compose_filename.blank?
                  t2.connect [
                               [
                                 "cd deploy && #{
                                   pod.app.env_configs.map(&:for_bash).join
                                 } docker-compose run #{hook}",
                                 "[ENV VARIABLES REDACTED...] docker-compose run #{
                                   hook
                                 }"
                               ]
                             ]
                  ...
                  t2.connect [
                               [
                                 "cd deploy && #{
                                   ...
                                   pod.compose_filename
                                 } run #{hook}",
                                 "[ENV VARIABLES REDACTED...] docker-compose run  -f #{
                                   pod.compose_filename
                                 } #{hook}"
                               ]
                             ]
            ...
            if build_vars.blank?
              t2.connect [
                           [
                             "cd deploy && #{pod.build_command}",
                             "HTTP_PROXY OFF:- #{pod.build_command}"
                           ]
                         ]
            else
              t2.connect [
                           [
                             "cd deploy && #{build_vars} #{pod.build_command}",
                             "WITH HTTP_PROXY... #{pod.build_command}"
                           ]
                         ]
            end
  ...
  def build_vars
    return(
      if ENV['BUILD_PROXY'].blank?
        ''
      else
        "HTTP_PROXY=#{ENV['BUILD_PROXY']} http_proxy=#{ENV['BUILD_PROXY']}"
      end
    )
  end
end
