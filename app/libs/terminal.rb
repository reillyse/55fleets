...
class Terminal
  ...
  def stream(command)
    Net::SSH.start(@host, @user, key_data: [@key], timeout: @timeout) do |ssh|
      ssh.exec!(command) { |channel, stream, data| Rails.logger.debug data }
    end
  end
    ...
    Net::SSH.start(@host, @user, key_data: [@key], timeout: @timeout) do |ssh|
      commands.each do |command|
        ...
        begin
          ...
          if out[1].match(
               'Service.*failed to build: The command.*returned a non-zero code:.*'
             )
            raise FailedCommandException, out[1]
          end
    ...
    begin
      result =
        Net::SCP.upload!(
          ...
          file,
          remote_file,
          ssh: { key_data: [@key] }, recursive: dir
        ) do |ch, name, sent, total|
          Rails.logger.debug "#{name}: (#{
                               ((sent.to_f / total.to_f) * 100).round(2)
                             }%)"
        end
  ...
  def ssh_exec!(ssh, command, stage = 'build', log_line = nil)
    ...
    ssh.open_channel do |channel|
      channel.exec(command) do |ch, success|
        ...
        LogEntry.write_log(
          stdout: 'executing:' + log_line,
          machine_id: @machine.id,
          pod_id: pod_or_nil(@machine),
          stage: stage
        )
        unless success
        ...
        channel.on_data do |ch, data|
          ...
          LogEntry.write_log stdout: data,
                             machine_id: @machine.id,
                             pod_id: pod_or_nil(@machine),
                             stage: stage
        end
        ...
        channel.on_extended_data do |ch, type, data|
          ...
          LogEntry.write_log stderr: data,
                             machine_id: @machine.id,
                             pod_id: pod_or_nil(@machine)
        end
        ...
        channel.on_request('exit-status') do |ch, data|
          ...
          LogEntry.write_log exit_code: data.read_long,
                             machine_id: @machine.id,
                             pod_id: pod_or_nil(@machine)
        end
        ...
        channel.on_request('exit-signal') do |ch, data|
          ...
          LogEntry.write_log exit_signal: data.read_long,
                             machine_id: @machine.id,
                             pod_id: pod_or_nil(@machine)
        end
      end
