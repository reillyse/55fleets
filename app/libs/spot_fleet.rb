class SpotFleet
  ...
  def get_history(spot_fleet_request_id, since_time = nil)
    ...
    return(
      client.describe_spot_fleet_request_history(
        { spot_fleet_request_id: spot_fleet_request_id, start_time: since_time }
      )
    )
  end

  def create_fleet_request(client_token,
  target_capacity,
  pod,
  subnets,
  instance_types = 'r5.large, m5.large, c4.large, c5.large',
  spot_price = '0.1',
  allocation_strategy = 'diversified')
    ...
    instance_types.split(',').map(&:strip).sort { rand }.each do |it|
      subnets.each do |subnet|
        launch_configs <<
          SpotFleet.launch_config_with_type(
            it,
            spot_price,
            ami_image,
            subnet.subnet_id,
            pod.app.deploy_key.name
          )
      end
    end
  ...
  def self.launch_config_with_type(instance_type,
  spot_price,
  ami_image,
  subnet_id,
  deploy_key)
