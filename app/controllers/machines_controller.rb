class MachinesController < ApplicationController
  ...
  def turn_on_logging
    ...
    if @machine.running? &&
         (@machine.logging_until.nil? || @machine.logging_until < Time.now)
    ...
    respond_to do |format|
      format.html do
        redirect_to app_fleet_path(@app, @machine.fleet) and return
      end
  ...
  def show
    ...
    unless current_user.apps.map(&:machines).flatten.compact.include? @machine
      raise NotAllowed
    end

    respond_to do |format|
      format.json do
        render status: :ok, json: @machine.as_json(include: :subnet) and return
      end
    end
  end
end
