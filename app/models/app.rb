class App < ActiveRecord::Base
  ...
  def latest_deployed_fleet
    self.fleets.where.not(rolling_deploy_completed_at: nil).order(
      'rolling_deploy_completed_at desc'
    ).first
  end
  ...
  def self.recreate_cache
    begin
      ...
      PoolManager.create_fleet(
        if ENV['BUILD_MACHINE_POOL_SIZE']
          ENV['BUILD_MACHINE_POOL_SIZE'].to_i
        else
          2
        end,
        build_cache.pods.first,
        ENV['CACHE_MACHINE_TYPES'] || 't1.micro'
      )
    end
  end
  ...
  def self.recreate_build_system
    kill_build_system
    PoolManager.create_fleet(
      ENV['BUILD_MACHINE_POOL_SIZE'] ? ENV['BUILD_MACHINE_POOL_SIZE'].to_i : 2,
      build_system.pods.first,
      ENV['BUILD_MACHINE_TYPES'] || 'c4.large'
    )
  end
  ...
  def for_react
    @fleets =
      (
        self.fleets.order('created_at desc').includes(pods: :machines).limit(
          20
        ) + self.fleets.with_running_machines.compact
      ).uniq
