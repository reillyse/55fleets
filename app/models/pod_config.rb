class PodConfig < ActiveRecord::Base
  INSTANCE_SIZES = %w[xsmall tiny small medium large jumbo custom]
  ...
  validates :instance_size,
            inclusion: {
              in: INSTANCE_SIZES, message: '%{value} is not a valid size'
            }
  ...
  def instance_type_from_instance_size
    if self.instance_type.blank?
      self.instance_type = PodConfig.size_to_instance(self.instance_size)
    end
  end
end
