class Pod < ActiveRecord::Base
  ...
  state_machine :state, initial: :created do
    event :build do
      transition %i[created] => :built
    end

    event :imaging_started do
      transition %i[built] => :imaging
    end

    event :imaged do
      transition %i[imaging] => :image_available
    end

    event :back_to_built do
      transition %i[imaging imaged] => :built
    end
  ...
  def initialize_machines
    ...
    number_of_machines_to_add =
      (self.permanent_minimum || 0) - self.on_demand_machines.running.count
    if number_of_machines_to_add > 0
      add_on_demand_machines number_of_machines_to_add
    end
  ...
  def add_on_demand_machines(instance_count)
    ...
    new_machines.each do |machine|
      after_transaction { Birther.perform_later(machine.id) }
    end
  end
  ...
  def next_subnet(excluded_subnets = [])
    subnet_count =
      (subnets - excluded_subnets).each_with_object(Hash.new(0)) do |s, count|
        count[s] = s.machines.running.count
      end
