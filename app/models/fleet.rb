class Fleet < ActiveRecord::Base
  ...
  scope :with_running_machines,
        lambda {
          joins(:pods, :machines).where(
            'machines.state' => %i[running in_action]
          )
        }
  scope :deployed, -> { where.not(rolling_deploy_completed_at: nil) }

  def launch(fleet_config)
    fleet_config.pod_configs.each do |p|
      self.pods.create! repo: p.repo,
                        compose_command: p.compose_command,
                        name: p.name,
                        number_of_members: p.number_of_members,
                        instance_type: p.instance_type,
                        app: self.app,
                        load_balanced: p.load_balanced,
                        compose_filename: p.compose_filename,
                        before_hooks: p.before_hooks,
                        after_hooks: p.after_hooks,
                        build_command: p.build_command,
                        git_ref: p.git_ref,
                        permanent_minimum: p.permanent_minimum,
                        spot_amount: p.number_of_members - p.permanent_minimum
    end
  end

  def is_most_recent?
    return(
      self.app.fleets.deployed.order('rolling_deploy_completed_at desc')
        .first == self
    )
  end
  ...
  def cleanup_when_finished
    self.pods.map(&:machines).flatten.select(&:running?).each(&:shutdown!)
    self.pods.map(&:spot_fleet_request).compact.select(&:active?).map(&:cancel!)
  end

  def post_deploy_hook; end
  ...
  def redeploy_this_fleet
    ...
    @new_fleet.save!
    @new_fleet.pods.each { |pod| PodInit.perform_later pod.id }
  end

  def for_react
    return(
      self.as_json.merge(
        pods: self.pods.order(:id).map(&:as_json),
        machines:
          self.machines.order(:id).map do |m|
            m.as_json(include: %i[pod subnet])
          end,
        running_count: self.machines.running.count,
