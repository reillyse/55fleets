class Machine < ActiveRecord::Base
  ...
  validates_uniqueness_of :ip_address,
                          ...
                          if:
                            Proc.new { |m|
                              Rails
                                .logger.debug '---------------------------------------------------------------------------------------------------- checking ip_address = m.ip_address'
                              m.running?
                            }
  ...
  def deregister_from_load_balancer
    ...
    Rails
      .logger.debug 'Finished queueing the deregister from load balancer, should be good'
  end

  def queue_for_reaping
    if self.is_a? SpotMachine
      Rails.logger.debug 'Not reaping spot machines' and return
    end
    Reaper.perform_later self.id
  end

  scope :running, -> { where(state: %i[running in_action]) }
  scope :starting, -> { where(state: :starting) }
  scope :operating, -> { where(state: %i[running in_action starting created]) }
  scope :free, -> { where(busy: [nil]) }
  scope :busy, -> { where.not(busy: [nil]) }

  def log(message, stage = 'system')
    LogEntry.create! stdout: message,
                     machine_id: self.id,
                     pod_id: pod ? pod.id : nil,
                     stage: stage
  end
  ...
  def clear_old_docker_images
    ...
    begin
      t.connect [
                  'df -h',
                  'docker images -f dangling=true -q | xargs -r docker -f rmi',
                  'df -h'
                ]
    rescue FailedCommandException => e
      Rails.logger.debug e.message + ' ... continuing'
  ...
  def create_instance
    ...
    begin
      self.ip_address, self.instance_id =
        Instance.create_instance self.instance_type,
                                 vpc.vpc_id,
    ...
    Rails
      .logger.warn '++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ Queueing for Deployer in Machine'
    Deployer.perform_later(self.id)
  end

  def kill_instance
    unless self.terminating? || self.terminated?
      raise 'Machine needs to be in terminating state'
    end
